 ...
 * Encoding procedures are romoved. You may find them in the
 * original package.
...
function LZHUnpack(Input, Output: TStream):Longint;
...
Function GetBit(Input:TStream): SmallInt;	{ get one bit }
VAR
	i: BYTE;
	i2 : SmallInt;
BEGIN
	...
	WHILE (getlen <= 8) DO
		BEGIN
			try
				Input.ReadBuffer(i, 1);
			except
				on EReadError do exit;        //reached the end of data
			end;
			i2 := i;
...
Function GetByte(Input:TStream): SmallInt;	{ get a byte }
VAR
  j : BYTE;
	i : WORD;
BEGIN
...
FUNCTION DecodeChar(Input:TStream): SmallInt;
VAR
	c : WORD;
	ret: SmallInt;
BEGIN
...
Function DecodePosition(Input:TStream) : SmallInt;
VAR
	i, j, c : SmallInt;
	ret: SmallInt;
BEGIN
