...
type
	TZ80Context = packed record
		case Integer of
			0: (F,A,C,B,E,D,L,H,PCL,PCH,SPL,SPH,IXL,IXH,IYL,IYH:Byte;);
			1: (AF,BC,DE,HL,PC,SP,IX,IY,AF_,BC_, DE_,HL_:Word;
					IFF1,IFF2,IM,R,I:Byte;
					);
	end;
	TZ80 = class (TCPU)
	private
		FIInt: TInterface;
		procedure IntChanged(NewValue, OldValue: Cardinal);
	protected
		...
		{$ENDIF}
	public
		FContext: TZ80Context;
		procedure UpdateContext; //Временная функция для отладчика
		...
		property Context:TZ80Context read FContext;
	end;
var
	MM: TMemoryMapper;
	StackMode, HALTE: Integer;
	IntStart: Integer;
...
const
	...
	F_ZERO=$40;				//Ноль
	F_SIGN=$80;				//Знак
	F_ALL=	F_CARRY + F_NEGATIVE + F_PARITY + F_3 + F_HALF_CARRY + F_5 + F_ZERO + F_SIGN;
...
procedure pushpc;
procedure Z80Reset;
procedure z80_execute(var local_tstates: integer);
{function Z80:integer;}
...
uses Windows, Forms, Sysutils, mmsystem, Math;
...
begin
    bitRes := val And (bit Xor $FFFF);     // 20090110
end;
...
begin
		//MemoryW(addr,newByte);
		MM.Write(addr, newByte);
end;
...
begin
    ...
    Case xxx of
      ...
      Else
        begin
        execute_cb := 8;
        //frmGL.OutMessage('Unknown CB instruction ' + inttostr(xxx) + ' at ' + inttostr(regPC));
        end;
    End;
end;
...
begin
    StackMode:=1;// Включить признак работы со стеком
    popw := peekb(regSP) Or (peekb(regSP + 1) * 256);
    StackMode:=0;// Выключить признак работы со стеком
...
begin
    ...
    Case xxx of
      ...
      Else
      begin
        //FrmGL.OutMessage('Unknown ED instruction ' + inttostr(xxx) + ' at ' + inttostr(regPC));
        execute_ed := 8;
      end;
    End;
end;
    ...
    Case xxx of
      ...
      203:  //prefix CB
        begin
          ...
          If ((op And $C0) = $40) Then execute_id := 20 Else execute_id := 24;
        end;
      225:  //POP ID
        begin
          regID := popw;
      ...
      begin
        execute_id := 8;
        //FrmGL.OutMessage('Unknown ID instruction ' + inttostr(xxx) + ' at ' + inttostr(regPC));
      end;
    End;
end;

// Обработчик комманд Z80
procedure z80_execute(var local_tstates: integer);
    var
      d : integer;
      lTemp : integer;
      ...
      Case xxx of
        ...
        227:  //EX (SP),HL
          begin
            ...
            pokew(regSP, lTemp);
            StackMode:=0;
            local_tstates := local_tstates + 20;
          end;
        235:  //EX DE,HL
          begin
            lTemp := regHL;
        ...
        255:  //RST 56
          begin
            ...
            local_tstates := local_tstates + 12;
				...
				//Else
				//  FrmGL.OutMessage('Unknown instruction ' + inttostr(xxx) + ' at ' + inttostr(regPC));
        End;
end;

// Инициализация массива флага паритета
...
{
// Обработчик одной команды процессора Z80
Function Z80:integer;
Begin
if IntStart=1 then
   Begin //Hardware Interrupt
   // If it's a maskable interrupt
   If intIFF1 = False Then Inc(CpuCycles,0)
      Else begin
           Case intIM of
                0, 1:
                begin
...
if HALTE=1 then Begin // Если в режиме останова (HALTE=1)
                Inc(CpuCycles,8);
                intRTemp := intRTemp + 2;
                Result:=0;
                Exit;
...
begin
	...
	FIInt := CreateInterface(1, 'int', MODE_R, IntChanged);
...
begin
	...
	if IntStart=1 then Begin //Hardware Interrupt
		If intIFF1 = False Then Inc(c,4)
		Else begin
			Case intIM of
				0, 1:begin
							pushpc;
	...
	if DebugMode=DEBUG_BRAKES then
		if CheckBreakPoint(regPC) then
			DebugMode := DEBUG_STOPPED;
end;

procedure TZ80.IntChanged(NewValue, OldValue: Cardinal);
begin
...
begin
	CRC := 0;
	W := getAF;
	W := W and $FFD7; //11010111
	CRC16_update(CRC, @W, 2);
	W := getBC;
	CRC16_update(CRC, @W, 2);
	CRC16_update(CRC, @regDE, 2);
	CRC16_update(CRC, @regHL, 2);
	CRC16_update(CRC, @regIX, 2);
	CRC16_update(CRC, @regIY, 2);
	CRC16_update(CRC, @regSP, 2);
	CRC16_update(CRC, @regPC, 2);
	//CRC16_update(CRC, @intR, 1);
	//CRC16_update(CRC, @intI, 1);
	CRC16_update(CRC, @im, 1);
	W := regAF_;
	W := W and $FFD7; //11010111
	CRC16_update(CRC, @W, 2);
	CRC16_update(CRC, @regBC_, 2);
	CRC16_update(CRC, @regDE_, 2);
	CRC16_update(CRC, @regHL_, 2);
	CRC16_update(CRC, @intIFF1, 1);
	CRC16_update(CRC, @intIFF2, 1);
	Result := CRC;
end;
{$ENDIF}

function CreateZ80r(IM:TInterfaceManager; ConfigDevice:TEmulatorConfigDevice):TComputerDevice;
begin
	Result := TZ80.Create(IM, ConfigDevice);
end;

begin
	RegisterDeviceCreateFunc('z80r', @CreateZ80r);
end.
